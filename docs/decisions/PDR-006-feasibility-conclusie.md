# PDR-006: Feasibility-onderzoek en conclusie — MCP en CLI als koppellaag
...
| Gerelateerd | PDR-001, PDR-005 |

## Hoe dit document te lezen

Dit document combineert de beslissing en het feasibility-onderzoek dat eronder
ligt. Wie alleen de uitkomst wil, leest [Context](#context), [Beslissing](#beslissing)
en [Consequenties](#consequenties). De [Onderbouwing](#onderbouwing-feasibility-onderzoek)
en [Bijlagen](#bijlage-a--validator-runs) zijn voor wie de meet- en
analysedetails wil zien.
...
Dit document beschrijft de bevindingen na zes weken werk; de volledige
onderbouwing staat verderop in [Onderbouwing](#onderbouwing-feasibility-onderzoek).

## Beslissing

Wij behouden beide transportmechanismen voorlopig, maar onder vier expliciete aanvullingen:

1. **De gedeelde standaard krijgt een derde profiel: CLI**. Provenance, audit en mutatie-bevestiging gelden onveranderd; alleen het transport verschilt. Zonder dit profiel is een geldige CLI-implementatie niet te toetsen. Het concrete tekstvoorstel hoort thuis in [`moza-mcp-standaard-poc`](https://github.com/MinBZK/moza-mcp-standaard-poc) (zie [Bijlage E](#bijlage-e--voorgestelde-uitbreiding-overheidsstandaard)).
2. **De host-orkestratielaag gaat `--fields` doorgeven aan CLI-tools.** Anders blijft de geadverteerde tokenwinst (97% reductie, zie [Bijlage B](#bijlage-b--token-metingen)) theoretisch.
3. **Een AI Act-classificatie en IAMA/DPIA worden ingepland** vóór de assistent breder dan een gesloten testgroep wordt aangeboden. De `rvo__indienen`-tool kan hoog risico zijn als de uitkomst directe juridische gevolgen heeft.

Wij maken nog géén keuze tussen MCP of CLI als productie-transport. Die beslissing pas wanneer de standaard is uitgebreid en de meetinfrastructuur (token-gebruik, latency, foutfrequentie) beter is getest.
...
1. **Werk in `moza-mcp-standaard-poc`**: concrete tekstvoorstellen liggen klaar in [Bijlage E](#bijlage-e--voorgestelde-uitbreiding-overheidsstandaard). Te bespreken met de standaardisatiegroep.
2. **Twee bugs te fixen in poc-moza**: KvK-resource (P1), KOOP-resource (P1). Geen workarounds nodig; één regel per server.
3. **Een wijziging in `cli_executor.py`**: `--fields` doorgeven, ontbrekende commando's toevoegen (`koop-cli regeling get`, `kvk-cli vestigingen`, `kvk-cli eigenaar`). Zie [Bijlage C](#bijlage-c--sync-gaten-host--cli).
...
## Onderbouwing: feasibility-onderzoek
...
Eén host, één LLM-orchestratie, twee transportlagen. Volledige stroom-diagrammen staan in [architecture.md](../architecture.md).
...
Vóór deze PR zag een nieuwkomer bovenaan `mcp/` (alleen een README met verouderde paden) náást `services/` (de echte implementatie). Twee PDRs waren als ongeldig gemarkeerd maar stonden ongedifferentieerd tussen de geldige; PDR-004 heette `structuur.md` maar bevatte een prompt voor het uitbreiden van de standaard. In deze PR is de `mcp/`-map verwijderd, PDR-004 hernoemd naar een sprekende naam, een index-README in `decisions/` toegevoegd, testvragen verplaatst naar `test-vragen.md`, en `.gitignore` opgeschoond. Zie de commit-historie van deze PR voor het volledige overzicht.
...
### Vergelijking op de vier dimensies
...
Beide kunnen voldoen aan provenance, audit en mutatie-bevestiging zoals de standaard voorschrijft (zie [Wat werkt](#wat-werkt)). De standaard zelf moet alleen worden uitgebreid om CLI als gelijkwaardig profiel te erkennen.
...
### AI Act, NeRDS, en validatiekader
...
**Wat ontbreekt** (samengevat uit deel E hierboven):
...
- `Functional provenance check`: slaat over als geen `--test-uri` is meegegeven; bij KvK met `--test-uri kvk://basisprofiel/68750110` resulteert dit in een ERROR door de bug in [Wat ontbreekt § A](#a-bug-in-de-kvk--en-koop-resource-implementatie-opgelost-in-deze-pr).
...
**Belangrijk**: de huidige host-implementatie geeft `--fields` nooit door aan CLI-tools (zie [Wat ontbreekt § B](#b-synchronisatie-tussen-host-en-cli-is-niet-automatisch)). De winst is dus pas reëel als wij dit doorvoeren: bijvoorbeeld door het LLM expliciet om velden te laten vragen, of door een vast veld-budget per tool te definiëren.
...
Het concrete tekstvoorstel voor de uitbreiding van de overheidsstandaard met
een CLI-profiel (toevoegingen aan §4.4, §7 en §8.3, plus nieuwe checks voor
het validatiepakket) hoort thuis in
[`moza-mcp-standaard-poc`](https://github.com/MinBZK/moza-mcp-standaard-poc),
niet in deze assistent-repo. Het wordt daar opgenomen als ADR en verwerkt in
de standaard-tekst zelf.

Het oorspronkelijke voorstel-document is verplaatst uit deze repo om de scope
zuiver te houden (zie ook PDR-004 voor de audit-trail).
